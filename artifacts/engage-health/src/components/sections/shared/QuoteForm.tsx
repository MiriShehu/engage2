...
import { Check, ArrowRight, ArrowLeft, Globe, Building2, Users, ChevronDown } from "lucide-react";
  ...
  return (
    <div ref={ref} className="relative flex-shrink-0">
      <button
        ...
        className="flex items-center gap-1.5 px-3 py-3 rounded-xl border border-border bg-muted text-sm font-medium text-secondary hover:border-primary/40 transition-colors whitespace-nowrap h-full"
      >
        ...
        <div className="absolute top-full left-0 mt-1 w-64 bg-white border border-border rounded-xl shadow-lg z-50 overflow-hidden">
          <div className="p-2 border-b border-border">
            <input
              ...
              className="w-full px-3 py-2 text-sm rounded-lg border border-border outline-none focus:border-primary"
            />
          </div>
          <div className="max-h-52 overflow-y-auto">
            {filtered.map((d, i) => (
              <button
                ...
                type="button"
                onClick={() => {
                  onChange(`${d.flag}${d.code}`);
                  setOpen(false);
                  setSearch("");
                }}
                className={cn(
                  "w-full flex items-center gap-2.5 px-3 py-2.5 text-sm hover:bg-primary/5 transition-colors text-left",
                  `${d.flag}${d.code}` === value && "bg-primary/10 text-primary font-medium"
                )}
              >
                <span className="text-base w-6">{d.flag}</span>
                <span className="flex-1 text-secondary">{d.name}</span>
                <span className="text-muted-foreground font-mono">{d.code}</span>
              </button>
            ))}
          </div>
        </div>
